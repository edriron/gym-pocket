...
import { Trash2, ChefHat, Zap } from 'lucide-react'
...
import { CalorieConverterDialog } from './CalorieConverterDialog'
...
export function DietRowItem({ row, dietTableId, canEdit }: DietRowItemProps) {
  ...
  return (
    <>
      <TableRow className="group hover:bg-muted/40 transition-colors">
        <TableCell className="font-medium py-2.5">
          <div className="flex items-center gap-2 truncate">
            <button
              onClick={() => nutrition && setConverterOpen(true)}
              title={nutrition ? 'Open calorie converter' : undefined}
              className="truncate text-left hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors disabled:cursor-default"
              disabled={!nutrition}
            >
              {name}
            </button>
            ...
            {nutrition && (
              <button
                ...
                title="Calorie converter"
                className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-emerald-600 dark:hover:text-emerald-400"
              >
      ...
      {nutrition && (
        <CalorieConverterDialog
          open={converterOpen}
          onOpenChange={setConverterOpen}
          sourceName={name}
          sourceCalories={nutrition.calories}
        />
      )}
    </>
  )
