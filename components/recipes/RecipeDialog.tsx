...
import type { Product, Recipe, RecipeWithIngredients, NutritionValues } from '@/types'
...
export function RecipeDialog({
  ...
  function addIngredient(item: { id: string; name: string; type: 'product' | 'recipe' }) {
  ...
  const nutrition: NutritionValues = sumNutrition(
    ingredients
      ...
      .map((ing) => {
        ...
        if (!product) return { calories: 0, carbs_g: 0, protein_g: 0, fats_g: 0 }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g. Protein Shake, Oatmeal Bowl" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
